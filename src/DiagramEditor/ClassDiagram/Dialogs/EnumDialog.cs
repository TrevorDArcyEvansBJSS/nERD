...
{
  public sealed class EnumDialog : ListDialog
  {
    private EnumType _parent = null;

    protected override void FillList()
    {
      lstItems.Items.Clear();
      foreach (EnumValue value in _parent.Values)
      {
    ...
    {
      EnumValue value = _parent.AddValue(text);
      ListViewItem item = lstItems.Items.Add(value.ToString());
    ...
    protected override void Modify(ListViewItem item, string text)
    {
      ...
      {
        EnumValue enumItem = _parent.ModifyValue((EnumValue)item.Tag, text);
        item.Tag = enumItem;
    ...
    protected override void Remove(ListViewItem item)
    {
      if (item != null && item.Tag is EnumValue)
        _parent.RemoveValue((EnumValue)item.Tag);
      base.Remove(item);
    }
